window.addEventListener("DOMContentLoaded", async () => {
    ...
    document.querySelector('#confirmRunModal .btn-submit')?.addEventListener('click', async () => {
        const modalEl = document.getElementById('confirmRunModal');
        const modal = bootstrap.Modal.getInstance(modalEl);
        modal?.hide(); // Close modal

        await handleSubmitScan(); // Only submit when confirmed
    });
    ...
    document.querySelector('#exampleModalCenter .btn-submit')?.addEventListener('click', async () => {
...
async function handleSubmitScan() {
    try {
        ...
        await submitScansToServer('2dac878d-5bf2-4e14-813d-a0fbc6655ba2', 'http://localhost:3000/api', selectedUrls, scheduledFor);
...
function collectScanFormData() {
    // Targets selected
    const selectedTargetRows = Array.from(document.querySelectorAll('#target-table tbody tr'))
...
function setupNewScanButton() {
    ...
    if (newScanBtn && newScanImg) {
        const updateImg = () => {
            newScanImg.src = isSelected ? "./images/new_scan.png" : "./images/new_scan_yellow.png";
        };

        updateImg();
        newScanBtn.addEventListener("mouseenter", () => {
            newScanImg.src = isSelected ? "./images/new_scan_yellow.png" : "./images/new_scan.png";
        });
...
function updateReviewSection() {
    ...
    const checkedScan = Array.from(document.querySelectorAll('#checkbox-list input[type="checkbox"]'))
        .find(cb => cb.checked);
    ...
    // 2. Targets
    const selectedRows = Array.from(document.querySelectorAll('#target-table tbody tr'))
